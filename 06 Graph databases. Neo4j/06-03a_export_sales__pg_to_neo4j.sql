...
CREATE TABLE sales__pg_to_neo4j (
	attrib varchar(5000)) ;


-- as in `sales` database the number of records is not so big, 
-- we can try to create the nodes and relationships in a single 
-- statement (which is not affordable when dealing with large databases)
DELETE FROM sales__pg_to_neo4j ;

INSERT INTO sales__pg_to_neo4j SELECT text FROM (
   ...
   SELECT 3 AS order, 
	...
	-- 4. Link `:Customer` nodes to `:PostalCode` nodes
   ...
   SELECT 4 AS order, 
	...
	-- 5. Create `:Category` nodes (Category names are composed of only letters, numbers and spaces)
   SELECT DISTINCT 5 AS order, 
	...
	-- 6. Link `:Product` nodes to `:Category` nodes
   SELECT 6 AS order, 
