...
//###################################################################################
//###	                 Queries involving a chain of relationships
//###################################################################################
...
//## 	Display the movies in which Keanu Reeves played with other actors than Carrie-Anne Moss

MATCH (keanu:Person) -[r1:ACTED_IN]-> (m:Movie) <-[r2:ACTED_IN]- (p:Person)
WHERE keanu.name = 'Keanu Reeves' AND
  NOT (m) <-[r3:ACTED_IN]- (carrie:Person {name :'Carrie-Anne Moss'}
RETURN *
