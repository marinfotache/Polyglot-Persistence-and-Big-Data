-- create tables of sales database in PostgreSQL
...
CREATE TABLE receipts (
    ...
    receiptDate DATE DEFAULT CURRENT_DATE
        CONSTRAINT ck_receiptDate CHECK (receiptDate >= TO_DATE('01/01/2016','DD/MM/YYYY')
            AND receiptDate <= TO_DATE('31/12/2020','DD/MM/YYYY')),
--  receiptType CHAR(1) CONSTRAINT ck_tipinc CHECK (tipinc IN ('P', 'C', 'A')),
    docType CHAR(4)
        CONSTRAINT ck_doctype CHECK(docType=UPPER(LTRIM(docType))),
    docNumber VARCHAR(16),
    docDate DATE DEFAULT CURRENT_DATE - 7
        CONSTRAINT ck_docDate CHECK (docDate >= TO_DATE('01/01/2016','DD/MM/YYYY')
            AND docDate <= TO_DATE('31/12/2020','DD/MM/YYYY'))
    ) ;
