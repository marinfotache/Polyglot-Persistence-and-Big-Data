...
-- I.
-- Connect to PgAdmin
-- create and populate database `sales` using scripts `neo-03a1_creating_tables__sales_PostgreSQL.sql` and 
-- `neo-03a2_populating_tables__sales_PostgreSQL.sql` on this github directory:
...
`neo-03b_export_sales_pg_to_neo4j.cypher`
...
// 2. Create a project called `project_sales`
// 3. Create a database (Local Graph) called `sales2025` (pay attention to the password)
// 5. Start (make active) the database `sales2025`
...
// (`neo-03b_export_sales_pg_to_neo4j.cypher`), paste it into the
