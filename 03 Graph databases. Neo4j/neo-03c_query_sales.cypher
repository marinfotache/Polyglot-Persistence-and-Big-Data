...
MATCH (county_IS:County { name: "Iasi"})
WITH county_IS
MATCH (pc:PostalCode) -[:PostalCodeInCounty]-> (county_IS)
WITH pc
MATCH (c:Customer) -[:LocatedIn]-> (pc)
WITH c
MATCH (i:Invoice) -[:SentTo]-> (c)
WITH i
MATCH (i)  -[:InvoiceDetails] ->  (p:Product)
WITH p
MATCH (p)  -[:ProductInCategory] ->  (categ:Category)
RETURN DISTINCT categ.category
ORDER BY  categ.category
